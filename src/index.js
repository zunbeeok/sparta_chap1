...
app.put('/member', (req, res) => {
    ...
    const info = req.body
    // db.updateMember(id)
    connection.query('UPDATE member SET name=? , mbti=?, advantage=?, introduction=?, goal=?, promise=?, git=?, pw=? WHERE id=?', [info.name, info.mbti, info.advantage, info.introduction, info.goal, info.promise, info.git, info.pw, id], (err, result, fields) => {
